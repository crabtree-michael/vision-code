...
#Preview {
    var root: PathNode {
        let file1 = File(path: "/michael/george/test.txt")
        let file2 = File(path: "/michael/george/long.txt")
        let file3 = File(path: "/michael/smith/john/text.txt")
        let file4 = File(path: "/michael/smith/text.txt")
        let file5 = File(path: "/michael/test.txt")
        
        return PathNode(file: File(path: "/michael"), subnodes: [
            PathNode(file: File(path: "/michael/george", icon: .folder, isFolder: true), subnodes: [
                PathNode(file: file1, subnodes: []),
                PathNode(file: file2, subnodes: [])]),
